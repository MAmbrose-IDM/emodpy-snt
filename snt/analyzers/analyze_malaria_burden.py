...
class monthlyU1PfPRAnalyzer(IAnalyzer):
    ...
    def map(self, data, simulation):
        ...
        for year, fname in zip(range(self.start_year, self.end_year), self.filenames):
            pop_all = data[fname]['DataByTimeAndAgeBins']['Average Population by Age Bin'][:12]
...
class monthlyU5PfPRAnalyzer(IAnalyzer):
    ...
    def map(self, data, simulation):
        ...
        for year, fname in zip(range(self.start_year, self.end_year + 1), self.filenames):
            ...
            # population size
            pop_all = data[fname]['DataByTimeAndAgeBins']['Average Population by Age Bin'][:12]  # remove final five days: assume final five days have same average as rest of month
...
class MonthlyNewInfectionsAnalyzer_byAgeGroup_withU1U5(IAnalyzer):

    def __init__(self, expt_name, sweep_variables=None, working_dir=".", start_year=2020, end_year=2026,
                 input_filename_base='MalariaSummaryReport_Monthly',
                 output_filename='newInfections_PfPR_cases_monthly_byAgeGroup_withU1U5.csv'):

        super(MonthlyNewInfectionsAnalyzer_byAgeGroup_withU1U5, self).__init__(working_dir=working_dir,
                                                                  filenames=[
    ...
    def map(self, data, simulation):
        ...
        for year, fname in zip(range(self.start_year, self.end_year + 1), self.filenames):
            ...
            severe_cases_Under5 = [((d[yy][0] * pop[yy][0]) + (d[yy][1] * pop[yy][1])) * 30 / 365 for yy
                                   in range(12)]
...
#
# class monthlySevereTreatedAnalyzer_byAgeGroup_withU1U5(IAnalyzer):
#
#     @classmethod
#     def monthparser(self, x):
#         if x == 0:
...
#             return datetime.datetime.strptime(str(x), '%j').month
#
#     def __init__(self, expt_name, event_name='Received_Severe_Treatment',
#                  sweep_variables=None, working_dir=".", start_year=2020, end_year=2026):
#
#         super(monthlySevereTreatedAnalyzer_byAgeGroup_withU1U5, self).__init__(working_dir=working_dir,
#                                                                 filenames=["output/ReportEventRecorder.csv"]
#                                                                 )
#         self.sweep_variables = sweep_variables or ["admin_name", "Run_Number"]
#         self.event_name = event_name
...
#         self.start_year = start_year
#         self.end_year = end_year
#         self.agebins_lower = [0, 0, 5, 15, 30, 50, 0]
#         self.agebins_upper = [1, 5, 15, 30, 50, 200, 200]
#         self.agebins_names = ['Under1', 'Under5','5to15', '15to30', '30to50', '50plus', 'allAges']
#
#     # def filter(self, simulation):
#     #     return simulation.tags['admin_name'] == 'Batie'
#
...
#             output_data.loc[:, 'year'] = output_data['Time'] // 365 + self.start_year
#             output_data.loc[:, 'age in years'] = output_data['Age'] / 365
#             for aa in range(len(self.agebins_names)):
#                 agemin = self.agebins_lower[aa]
#                 agemax = self.agebins_upper[aa]
#                 agelabel = self.agebins_names[aa]
#                 d = output_data[(output_data['age in years'] < agemax) & (output_data['age in years'] > agemin)]
#                 g = d.groupby(['year', 'month'])['Event_Name'].agg(len).reset_index()
...
#                 else:
#                     if not g.empty:
#                         simdata = pd.merge(left=simdata, right=g, on=['year', 'month', 'AgeGroup', 'Num_Received_Severe_Treatment'], how='outer')
#                         simdata = simdata.fillna(0)
#
#
#             for sweep_var in self.sweep_variables:
...
#                     simdata[sweep_var] = simulation.tags[sweep_var]
#         else:
#             simdata = pd.DataFrame(columns=['year', 'month', 'AgeGroup', 'Num_Received_Severe_Treatment'] + self.sweep_variables)
#         return simdata
#
#     def reduce(self, all_data):
#
...
#         adf = pd.concat(selected).reset_index(drop=True)
#         adf = adf.fillna(0)
#         adf.to_csv(os.path.join(self.working_dir, self.expt_name, 'Treated_Severe_Monthly_Cases_By_AgeGroup_withU1U5.csv'),
#                    index=False)
#         severe_treat_df = adf
#         # cast to int65 data type for merge with incidence df
#         severe_treat_df = severe_treat_df.astype({'month': 'int64', 'year': 'int64', 'Run_Number': 'int64'})
...
#         incidence_df = pd.read_csv(os.path.join(self.working_dir, self.expt_name, 'newInfections_PfPR_cases_monthly_byAgeGroup_withU1U5.csv'))
#         merged_df = pd.merge(left=incidence_df, right=severe_treat_df,
#                              on=['admin_name', 'year', 'month', 'Run_Number', 'AgeGroup'],
#                              how='left')
#         merged_df = merged_df.fillna(0)
#
#         # fix any excess treated cases!
#         merged_df['excess sev treat'] = merged_df['Num_Received_Severe_Treatment'] - merged_df['Severe cases']
#
#         for (rn, admin_name, age_group), rdf in merged_df.groupby(['Run_Number', 'admin_name', 'AgeGroup']):
#             for r, row in rdf.iterrows():
...
#                 if row['year'] == self.start_year and row['month'] == 1:
#                     merged_df.loc[(merged_df['year'] == self.start_year) & (merged_df['month'] == 1) & (
#                             merged_df['Run_Number'] == rn) & (merged_df['admin_name'] == admin_name) & (merged_df['AgeGroup'] == age_group),
#                                   'Num_Received_Severe_Treatment'] = np.sum(
#                         merged_df[(merged_df['year'] == self.start_year) &
#                                   (merged_df['month'] == 1) &
#                                   (merged_df['Run_Number'] == rn) &
#                                   (merged_df['AgeGroup'] == age_group) &
#                                   (merged_df['admin_name'] == admin_name)]['Severe cases'])
#                 else:
#                     # figure out which is previous month
#                     newyear = row['year']
#                     newmonth = row['month'] - 1
...
#                     merged_df.loc[(merged_df['year'] == self.start_year) & (merged_df['month'] == 1) & (
#                             merged_df['Run_Number'] == rn) & (merged_df['admin_name'] == admin_name) & (merged_df['AgeGroup'] == age_group), 'Num_Received_Severe_Treatment'] = \
#                         merged_df.loc[(merged_df['year'] == self.start_year) & (merged_df['month'] == 1) & (
#                                 merged_df['Run_Number'] == rn) & (merged_df['admin_name'] == admin_name) & (merged_df['AgeGroup'] == age_group),
...
#                                                       merged_df['Severe cases']
#         merged_df.loc[
#             merged_df['excess sev treat'] > 0.5, 'Num_Received_Severe_Treatment'] = \
#             merged_df.loc[merged_df['excess sev treat'] > 0.5, 'Severe cases']
#
#         # del merged_df['Severe cases']
#         del merged_df['excess sev treat']
#         merged_df.to_csv(os.path.join(self.working_dir, self.expt_name,
#                                       'newInfections_PfPR_cases_treat_byAgeGroup_withU1U5.csv'), index=False)
